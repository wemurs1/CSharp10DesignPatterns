...
public class AddEmployeeToManagerList : ICommand
{
    ...
    public bool CanExecute()
    {
        ...
        if (_employeeManagerRepository.HasEmployee(_managerId, _employee.Id)) return false;

        // other potential rule(s): ensure that the employee can only be added to one manager's list at a time
        return true;
    }
...
public class CommandManager
{
    ...
    public void Invoke(ICommand command)
    {
        ...
        if (_command == null) _command = (AddEmployeeToManagerList)command;
